...
public class BaseDataAndCellTableConfig extends SuperConfig {
	...
	public void findInvalidColumn(ArrayList<Integer> columnValues) {
		for(Integer columnValue:columnValues){
			if(columnValue==0){
				...
				neVersion="-1";
			}else if(columnValue==10){
				imsi="-1";
			}else if(columnValue==11){
				hier3_ID="-1";
			}else if(columnValue==12){
				hier32_ID="-1";
			}else if(columnValue==13){
				hier321_ID="-1";
			}
		}
	...
	public static void storeErrors(String columnDescription, String cellContents){
		...
		try{
			...
			pw.write(columnDescription+", ");
			pw.write(cellContents+", ");
			pw.println();
			pw.close();
		}
		catch(IOException e){
	...
	public static void storeErrors(String columnDescription, String cellContents1, String cellContents2){
		...
		try{
			pw=new PrintWriter(new FileWriter("errorLog.csv",true));
			pw.write(columnDescription+", ");
			pw.write(cellContents1+", ");
			pw.write(cellContents2+", ");
			pw.println();
			pw.close();
		}
		catch(IOException e){
