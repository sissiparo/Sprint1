...
public class BaseDataAndCellTableConfig extends SuperConfig {
	...
	public void getObjects(){
		...
		eventcause = PersistenceUtil.findEventID(Integer.parseInt(eventID));
		mccmnc = PersistenceUtil.findMCCMNCByName(Integer.parseInt(mncID));
		celltable = PersistenceUtil.findByCellID(Integer.parseInt(cellID));
		failure = PersistenceUtil.findFailureCode(Integer.parseInt(failureClassID));
		userequipment = PersistenceUtil.findTAC(TAC);
	...
	public static void storeRowError(String[] rowRemoved){
		...
		try{
			...
			System.out.println("Error whilst updating error log: "+e.getMessage());
		}
	}
